...
**SPM note:** `ios/App/CapApp-SPM/Package.swift` is regenerated by `npx cap sync ios`. If Xcode shows *"Missing or empty JSON output from manifest compilation for capapp-spm"*, run `npm install` then `npm run ios:sync` on your Mac (paths must use forward slashes, not Windows `\\`), then in Xcode: **File → Packages → Reset Package Caches** → **Resolve Package Versions** → **Product → Clean Build Folder**.
