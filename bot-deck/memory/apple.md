# Apple bot memory

Owns consumer visual chrome (Apple HIG / Picasso). Playbook:
`.cursor/skills/kepi-apple-bot/SKILL.md`

## Standing

- Light default (`#F5F5F7` / `#1D1D1F`). Gold CTA `#f4c95d`. Lucide, not emoji.
- Tokens: `src/lib/ui/appleDesign.ts` + `src/app/globals.css` `--apple-*`
- Discuss-before-code on “how should this look?”
- Not a Bot Deck assignee id (same as points/card) — conductor routes here

## Do not

- Navy empty Home / Plan cockpits
- GitHub-dark share view as the family-facing surface
- Raw ML scores or ISO datetime labels in consumer repair UI
