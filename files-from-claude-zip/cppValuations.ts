...
// v2: BUG FIXES
//  (1) passenger-aware: award miles/surcharge are per-person, cash totals are
//      all-passenger. Every comparison now scales the award by passenger count.
//  (2) cabin-matched: the orchestrator only ever passes a SAME-CABIN cash fare
//      as the benchmark, so a business award is never compared to economy cash.
...
// Cash-equivalent of an award FOR THE WHOLE PARTY:
//   (milesPerPax * pax * cpp) + (surchargePerPax * pax)
...
// Value extracted per point, measured against a CABIN-MATCHED cash fare.
// Both sides are whole-party totals, so the ratio is correct for any pax count.
export function realizedCpp(
  ...
  const cashYouAvoid =
    comparableCashOffer.totalAmount - award.cashSurcharge * pax;
    ...
    return {
      ...
      reason:
        ...
        (award.surchargeHeavy
          ? " Note: surcharge-heavy program, already included above."
          : ""),
    };
  }
