...
The airport view renders the **real OpenStreetMap raster basemap** (`buildOsmRasterFallbackStyle()` → `tile.openstreetmap.org`, already in CSP `connect-src`, no API key, free) so travelers see the true terminal shape, parking, roads and taxiways drawn to scale — "the map from that site." It is flat/top-down (`pitch 0, bearing 0`), pinch-zoomable, with an explicit `NavigationControl` (+/−) and the required OSM `AttributionControl`; `minZoom 12 / maxZoom 19`. Our overlays sit **on top** of the basemap: a faint tint + crisp outline of the traveler's terminal footprint (`kepi-zones-fill`/`-outline` — do NOT redraw buildings/roads, OSM already does), the walking route, POI markers and the user puck. The camera `fitBounds` to the layout's real geometry (`computeLayoutBounds`), not a fixed center/zoom. **The light SVG floor plan (`AirportSchematicLayer`) always renders underneath the map as the base layer** (z-1 below the map's z-2): it paints instantly, works offline, and if tiles or the WebGL context ever fail the screen **never blanks** — it falls back to the floor plan. The map is used in **both** planning (`previewMode`) and at-airport. To avoid the old family→airport WebGL context-limit blank, map creation is deferred a frame (the async `import("maplibre-gl")` + the `/live-map` family teardown on `mapView==="airport"` already order teardown before init) and the schematic underneath guarantees no blank. **Never** use MapTiler for the airport basemap (OSM only, per owner intent + cost); never remove the SVG fallback layer.
