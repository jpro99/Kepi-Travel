...
**M71 — Indoor airport GPS still shows you on the map**
Preview/plan mode must **never hide** the traveler GPS puck when `userLat`/`userLon` exist — indoor lounge GPS (e.g. FCO club) often fails strict geofence or graph snap but the traveler still needs to see themselves and tap **I'm here** to pin a lounge. Use `resolveTravelerDisplayPosition` (raw GPS unless confirmed node); `isNearAirportIata` slack (1.35× radius) promotes Live Map to live mode when GPS is near the displayed airport. Journey prompts must accept **typed** replies (not buttons-only) plus **Skip for now**.

**Test:** `src/lib/airportNav/airportTravelerGpsPuck.test.ts`, `src/lib/travelAssistant/airportGeo.test.ts`
