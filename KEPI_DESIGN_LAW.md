...
United / MileagePlus confirmations show `Total 24,000 miles + 195.80 USD` — parse the grand total for trip spend (cash taxes + miles redeemed). Never read `12,000` as $12. Multi-leg bookings on one confirmation dedupe miles/cash once. Confirmations untouched.

**Test:** `src/lib/travelAssistant/parseReservationPricing.test.ts`
...
| G33 | `src/lib/travelAssistant/parseReservationPricing.test.ts` |
