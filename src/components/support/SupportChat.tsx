...
export function SupportChat() {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent(SUPPORT_CHAT_STATE_EVENT, { detail: { open: isOpen } }),
    );
  }, [isOpen]);
  ...
  return (
    <>
      ...
      {isOpen ? (
        <section
          className={`fixed inset-0 ${SUPPORT_CHAT_PANEL_Z_CLASS} flex bg-slate-950/75 sm:inset-auto sm:bottom-24 sm:right-6 sm:h-[480px] sm:w-[320px] sm:rounded-2xl sm:border sm:border-slate-700 sm:bg-slate-950/95`}
          data-testid="support-chat-panel"
        >
          <div className="flex h-[100dvh] max-h-[100dvh] w-full flex-col sm:h-full sm:max-h-none">
            ...
            <footer
              className="shrink-0 border-t border-slate-700 px-3 py-3"
              style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" }}
            >
              {error ? <p className="mb-2 text-xs text-rose-300">{error}</p> : null}
              <p className="mb-2 text-[11px] font-medium text-slate-400">{t("askAnythingLabel")}</p>
              <div className="flex gap-2">
                <input
                  ...
                  className="min-h-[44px] flex-1 rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-base text-slate-100 outline-none ring-cyan-300 focus-visible:ring-2"
                />
                <button
                  type="button"
