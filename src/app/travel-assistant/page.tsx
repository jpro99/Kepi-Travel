...
import { openSupportChat, SUPPORT_CHAT_STATE_EVENT } from "@/components/support/SupportChat";
...
export default function TravelAssistantPage() {
  ...
  useEffect(() => {
    const onSupportChatState = (event: Event): void => {
      const detail = (event as CustomEvent<{ open?: boolean }>).detail;
      setSupportChatOpen(Boolean(detail?.open));
    };
    window.addEventListener(SUPPORT_CHAT_STATE_EVENT, onSupportChatState);
    return () => window.removeEventListener(SUPPORT_CHAT_STATE_EVENT, onSupportChatState);
  }, []);
  ...
  // I61: SW register + controllerchange reload live only in <DeployRefresh />
  // (root layout). A second reload here raced DeployRefresh and blanked the
  // home-screen PWA after deploys / cache updates.
  ...
  const handleRescanImports = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const pricingUpdated = payload.pricingUpdatedCount ?? 0;
      const updated = payload.updatedReservations ?? 0;
      const sources = payload.rescannedSources ?? 0;
      if (pricingUpdated > 0) {
        const summary = `Updated pricing on ${pricingUpdated} booking${pricingUpdated === 1 ? "" : "s"} from ${sources} saved confirmation${sources === 1 ? "" : "s"}.`;
        setRescanImportsSummary(summary);
        setToast(`Re-scan logged prices on ${pricingUpdated} booking${pricingUpdated === 1 ? "" : "s"}.`);
      } else if (updated > 0) {
        ...
        <div className="mx-auto max-w-3xl space-y-4 px-3 py-3 sm:max-w-4xl sm:px-4 lg:max-w-6xl lg:px-5 xl:max-w-7xl">
          ...
          {!isCompactViewport && (showTripShellSkeleton ? (
            ...
            <ItineraryTabView
              ...
              itineraryPlans={itineraryPrefs.itineraryPlans}
              onPasteDayPlan={handlePasteDayPlan}
