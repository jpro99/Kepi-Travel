...
export function flightCardTypography(simplifiedMobile = false) {
  ...
  return {
    ...
    detailLabel: `${phone === "" ? "text-base" : "text-base lg:text-[10px]"} font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400 ${phone}tracking-widest`,
    detailValue: `${phone === "" ? "text-2xl" : "text-2xl lg:text-sm"} font-bold ${phone === "" ? "mt-1" : "mt-1 lg:mt-0.5"}`,
    actionBtn: `${phone === "" ? "py-3.5 text-base min-h-[52px]" : "py-3.5 text-base min-h-[52px] lg:py-2 lg:text-sm lg:min-h-0"} flex-1 rounded-2xl font-bold active:opacity-70 lg:rounded-xl lg:font-semibold`,
    airportCode: `${phone === "" ? "text-5xl" : "text-5xl lg:text-4xl"} font-black text-slate-900 dark:text-white tracking-tight leading-none`,
    timeText: `${phone === "" ? "text-lg" : "text-lg lg:text-base"} font-semibold text-slate-900 dark:text-white ${phone === "" ? "mt-1.5" : "mt-1.5 lg:mt-1"}`,
    dateText: `${phone === "" ? "text-base" : "text-base lg:text-sm"} font-bold text-slate-700 dark:text-slate-200 text-center leading-tight whitespace-nowrap`,
    ...
    airline: `${phone === "" ? "text-sm" : "text-sm lg:text-xs"} font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider`,
    flightNum: `${phone === "" ? "text-xs" : "text-xs lg:text-[10px]"} rounded-md bg-slate-100 dark:bg-slate-800 px-2 py-0.5 font-bold text-slate-600 dark:text-slate-300`,
...
export function guideCardTypography(simplifiedMobile = false) {
  const phone = simplifiedMobile ? "" : "lg:";
  return {
    eyebrow: `${phone === "" ? "text-sm" : "text-sm lg:text-[10px]"} font-bold uppercase tracking-wide text-sky-300/70 ${phone}tracking-widest`,
    gateLabel: `${phone === "" ? "text-sm" : "text-sm lg:text-[9px]"} font-bold uppercase tracking-wide text-sky-200/60 ${phone}tracking-widest`,
    gateValue: `${phone === "" ? "text-3xl" : "text-3xl lg:text-xl"} font-black mt-0.5`,
    gatePad: `${phone === "" ? "p-4" : "p-4 lg:p-3"}`,
    refreshBtn: `${phone === "" ? "py-4 text-base" : "py-4 text-base lg:py-2.5 lg:text-xs"}`,
  };
}
