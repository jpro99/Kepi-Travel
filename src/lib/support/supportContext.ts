...
function buildConnectionHints(reservations: TransportRouteReservation[]): string[] {
  ...
  for (const inbound of flights) {
    const hub = inbound.flightArrivalAirport?.trim().toUpperCase();
    if (!hub) continue;
    const ctx = resolveArrivalHubConnection(reservations, hub, inbound.id);
    if (!ctx) continue;
    const line = ctx.selfTransfer
