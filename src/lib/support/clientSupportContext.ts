/**
 * Live traveler context for Kepi Help — written by the travel shell,
 * read when the user opens support chat.
 */
...
export type SupportLiveContext = {
  ...
  /** Latest traveler-observed airport facts (never official FIDS). */
...
export function formatClientSupportContext(): string {
  ...
  if (ctx.todayKey?.trim()) lines.push(`Traveler calendar today: ${ctx.todayKey.trim()}`);
  if (ctx.journeyPhase?.trim()) lines.push(`Journey phase: ${ctx.journeyPhase.trim()}`);
  if (ctx.locationStatus?.trim()) lines.push(`Location status: ${ctx.locationStatus.trim()}`);
